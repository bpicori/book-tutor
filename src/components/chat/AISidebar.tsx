...
const TAB_CONFIG: { id: AiSidebarTab; label: string; icon: string; description: string }[] = [
  { id: 'preview', label: 'Preview', icon: 'visibility', description: 'Orient before reading' },
  { id: 'ask', label: 'Ask', icon: 'chat', description: 'Q&A after reading' },
]

export const AISidebar = memo(function AISidebar() {
  const { isAiSidebarOpen, toggleAiSidebar, activeAiTab, setActiveAiTab } = useStore()
    ...
    <aside className="flex flex-col w-96 h-full bg-warm-off-white border-l border-border-warm overflow-hidden">
      ...
      <header className="flex-shrink-0 p-4 border-b border-border-warm">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-muted-gray-text font-semibold text-base">AI Reading Guide</h3>
          <button
