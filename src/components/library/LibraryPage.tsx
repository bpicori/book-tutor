...
import { LoadingSpinner, Logo } from '../common'
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    ...
    try {
      ...
      const view = document.createElement('foliate-view') as HTMLElement & {
        book?: { metadata?: { title?: unknown; author?: unknown }; getCover?(): Promise<Blob | null> }
        open(file: File): Promise<void>
      }
      view.style.cssText = 'position:absolute;left:-9999px;width:1px;height:1px'
  ...
  return (
    <div className="min-h-screen bg-warm-off-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <div className="mb-8">
          <h2 className="text-xl font-semibold text-muted-gray-text mb-1">Your Library</h2>
          <p className="text-light-gray-text text-sm">
            {library.length === 0 ? 'Add your first book to get started' : `${library.length} book${library.length === 1 ? '' : 's'}`}
          </p>
        </div>
        ...
        {library.length === 0 && (
          <div className="text-center py-16">
            <span className="material-symbols-outlined text-6xl text-border-warm mb-4">library_books</span>
            <h3 className="text-muted-gray-text font-medium mb-2">Your library is empty</h3>
